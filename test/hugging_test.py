...
def test(pdf_paths, annotated_tables, draw='cell_match', tol=5, only_bbox=False, find_method='rule-based'):
    ...
    for pdf_path in pdf_paths:
        ...
        if only_bbox:
            ...
            try: 
                tableExtractor = TableExtractor(path=f"{dataset_path}/pdf/{pdf_path}", separate_units=False, find_method=find_method, model=model, determine_row_space="min", max_column_space=4, max_row_space=2)
